...
## Bug 12: Cannot zoom OUT when auto-zoom is active (OPEN)
When auto-zoom is enabled and has zoomed into a track segment, the user can pan and zoom IN further, but cannot zoom OUT to see more of the track or the full track. The zoom-out gesture appears to do nothing.
**Probable cause**: The auto-zoom sets the base transform to fit the segment bounds. The user's zoom scale composes on top of this base. Since the base is already zoomed in (scale > 1 relative to full track), the `MIN_SCALE = 1` limit in `mapInteraction.js` prevents zooming out far enough to see the full track. The user can only zoom in from the auto-zoomed position, not out.
**Investigate**: Check if `MIN_SCALE = 1` in `mapInteraction.js` is the limiting factor. Consider allowing the user to zoom out to a scale < 1 relative to the auto-zoomed base, or provide a "zoom to full track" gesture (e.g., double-click should return to full track even when auto-zoom is on — see Bug 5).
