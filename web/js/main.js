...
const PARTIAL_DIST_FRAC = 0.95;  // maxD must be ≥ 95% of trackLen
const PARTIAL_DUR_FRAC  = 0.5;   // duration must be ≥ 50% of median complete-lap dur
const ROLLING_DIST_M    = 50;    // |minD| at the first frame may be up to ±50 m
...
// ── Panel order (F9) ─────────────────────────────────────────────────────────
// panelOrder imported from appState.js
...
function renderPanel(def, bins, maxDist, sectorDists, zoomRange) {
  const H = Math.round(def.height * (def.heightMultiplier ?? 1.0));  // F11
...
function renderDtPanel(def, dtBins, maxDist, sectorDists, zoomRange, overlapRange) {
  const H = def.height;
  const plotH = H - PAD.top - PAD.bottom;
...
// getCurrentMapMode and setCurrentMapMode imported from appState.js
...
function renderCircuitMap() {
  ...
  if (getCurrentMapMode() === 'outline') {
    ...
    segGroup.innerHTML = renderHeatmapSegments(getCurrentMapMode());
  }
...
function renderMapLegend() {
  ...
  if (!el) return;
  if (getCurrentMapMode() === 'outline') { el.innerHTML = ''; return; }
  const col = HEATMAP_CHANNELS[getCurrentMapMode()];
  const bins = currentSessionBins?.[col];
  let lo = 0, hi = 1, unit = '';
  if (getCurrentMapMode() === 'speed' && bins) {
    lo = Infinity; hi = -Infinity;
    for (const v of bins) { if (isFinite(v)) { if (v < lo) lo = v; if (v > hi) hi = v; } }
    if (!isFinite(lo) || hi <= lo) { lo = 0; hi = 1; }
    unit = ' km/h';
  } else if (getCurrentMapMode() === 'brake' || getCurrentMapMode() === 'throttle') {
    lo = 0; hi = 1; unit = '';
  }
  const ramp = HEATMAP_RAMPS[getCurrentMapMode()];
  const stops = [];
  for (let i = 0; i <= 10; i++) stops.push(`${ramp(i / 10)} ${i * 10}%`);
  const fmt = getCurrentMapMode() === 'speed' ? v => v.toFixed(0) : v => v.toFixed(1);
...
async function loadDeltabestCsv(file) {
  ...
  try {
    ...
    if (dist.length < 10) throw new Error(`only ${dist.length} valid rows`);
    ...
    const segments = [{ lapNum: 0, start: 0, end: n }];
    annotateSegments(segments, lapDist, lapTime);
    const sidecar = {
    ...
    setBadge(badge, 'ok', `${n} rows · TinyPedal deltabest`);
    rebuildPickers();
...
document.getElementById('panels').addEventListener('mousedown', e => {
  const handle = e.target.closest('.drag-handle');
  if (!handle) return;
  const wrap = handle.closest('.panel-wrap');
  if (wrap) wrap.setAttribute('draggable', 'true');
});
document.getElementById('panels').addEventListener('mouseup', () => {
  // Catches the case where the user clicked the handle but didn't drag.
  disarmAllPanels();
});
document.getElementById('panels').addEventListener('dragstart', e => {
...
document.getElementById('panels').addEventListener('dragleave', e => {
...
document.getElementById('panels').addEventListener('dragend', () => {
  document.querySelectorAll('#panels .panel-wrap.drag-over').forEach(el => el.classList.remove('drag-over'));
  disarmAllPanels();
  state.dragId = null;
});
