...
| `10-lap-time-s-undercount` | `max(lap_time_s)` per segment underestimates true lap time by 0–180 ms — `lap_time_s` resets to negative at `lap_number` change | 📋 10b evaluated (77–176 ms confirmed); 10c prompt ready — fix `extract_reference_lap.py` to use `scoring_last_lap_time_s` from next segment |
