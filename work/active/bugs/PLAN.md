...
| `13-lap-time-undercount-full-stack` | `max(lap_time_s)` undercount (77–176 ms) still used by `lap_comparator.py`, `summary.py`, and `pipeline.js` `annotateSegments()` — `extract_reference_lap.py` was fixed in 10c but no other consumer was updated | ✅ Fixed in `58f9f8b` — pending live confirmation before moving to completed |
