...
function isStaleStartBoundaryFrame(lapTime, lapDist, trackLen) {
  return Number.isFinite(lapTime)
    && Number.isFinite(lapDist)
    && lapTime < 0.5
    && lapDist > trackLen * 0.5;
}
...
export function annotateSegments(segments, distances, lapTimes, scoringLastLapTime = null) {
  ...
  for (const seg of segments) {
    for (let i = seg.start; i < seg.end; i++) {
      const d = distances[i];
      if (Number.isFinite(d) && d > trackLen) trackLen = d;
    }
  }
  for (const seg of segments) {
    ...
    for (let i = seg.start; i < seg.end; i++) {
      ...
      if (isStaleStartBoundaryFrame(t, d, trackLen)) continue;
