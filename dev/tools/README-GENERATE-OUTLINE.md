...
The `dev/scripts/average_trajectory_outline.py` script generates a schema v1 track outline by:
...
The script loads parquet files from `dev/sessions/` directory. Each session contains telemetry data with:
...
python3 dev/scripts/explore_and_export_laps.py dev/sessions/session_*.parquet --fastest 1
...
5. Register outline: `python3 dev/scripts/register_outline.py <outline.json>` (also regenerates ES module)
