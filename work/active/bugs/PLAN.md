...
| `16-slow-pitstop-lap-passes-guard` | Lap 13 (84.42 s, 18.5% over 71.242 s reference) passes the distance-coverage guard (full lap) but the pitstop causes a 12.5 s phantom Δt at turn 4 → `compare_laps` would coach "You lost twelve seconds at turn 4". Need a duration guard for implausibly slow laps. | ✅ Fixed in `21155b0` → moved to completed |
| `17-js-stale-boundary-frame-partial-lap` | JS `annotateSegments()` computes `maxDist` from stale negative-time boundary frames, so lap 19 in the Bahrain Outer repro looks full-distance and gets the fastest-lap star even though the real trace ends near 2416m | 🐛 Open |
| `25-prefix-match-false-positive-layout-variant` | Coaching fires on tracks with no model — `track_model_resolver` and `reference_resolver` prefix-match `fuji-speedway-classic` to `fuji-speedway` data (different layout, same prefix) | ✅ Fixed in `c08e82d` |
| `26-track-outline-not-visible` | Static track outlines are not visible in the compare.html viewer (outline data, manifest, lookup, and rendering all verified working; root cause not yet found) | 🐛 Open |
