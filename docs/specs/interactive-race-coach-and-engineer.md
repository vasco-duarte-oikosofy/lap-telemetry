...
5. **Deterministic analysis engine**
   ...
   **Known simplifications to address in later slices:**
   ...
   - **Gain distance deltas (slice 01c.2 — in progress):** Currently, `top_gains` reports speed deltas measured at the driver's phase distance only. For losses this is sufficient (the driver's action is being corrected), but for gains the most actionable fact is the **distance delta** — how much earlier/later the driver entered/exited compared to the reference. This requires running entry/exit detection on the **reference** lap's throttle/brake traces too. Three spec documents define the algorithms:
     - [`[work folder]/interactive-race-coach/01c-determine-entry-exit-phase-algorithm/01c.2_exit_gains_improvements/apex_min_speed_gain_algorithm.md`](apex_min_speed_gain_algorithm.md) — adds `apex_offset_m`, delta-time gains for minimum_speed + exit, `gain_end_distance_m`. **Known issue: `compute_delta_time_trace()` must use `lap_time_s` directly (matching the web JS in `product/web/js/pipeline.js` `computeDeltaT()`), not speed integration.**
     - [`[work folder]/interactive-race-coach/01c-determine-entry-exit-phase-algorithm/01c.2_exit_gains_improvements/entry_gain_algorithm.md`](entry_gain_algorithm.md) — adds reference entry detection and `entry_distance_delta_m`.
     - [`[work folder]/interactive-race-coach/01c-determine-entry-exit-phase-algorithm/01c.2_exit_gains_improvements/exit_gain_algorithm.md`](exit_gain_algorithm.md) — adds reference exit detection and `exit_distance_delta_m`.
