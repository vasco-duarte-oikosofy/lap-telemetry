...
| `25-prefix-match-false-positive-layout-variant` | Coaching fires on tracks with no model — `track_model_resolver` and `reference_resolver` prefix-match `fuji-speedway-classic` to `fuji-speedway` data (different layout, same prefix) | 🐛 Open |
