...
async function runTests() {
  ...
  {
    const p07 = spawnSync('node', [path.join(ROOT, 'dev/scripts/test_width_profile_export.js')], {
      encoding: 'utf8', timeout: 60000,
    });
    assert(p07.status === 0, 'Phase 07 tests still pass');

    const p08 = spawnSync('node', [path.join(ROOT, 'dev/scripts/test_width_profile_confidence.js')], {
      encoding: 'utf8', timeout: 60000,
    });
